...
class socket{
  ...
  init() {
    ...
    //设置连接路径
    let {url, port} = params
    let global_params = ['heartBeat','heartMsg','reconnect','reconnectTime','reconnectTimes', 'onmessage']
    ...
    console.log("new WebSocket this.ws", this.ws)
  ...
  onerror(func) { 
    this.ws.onerror = event => {
      console.log("______11111111_______")
      ElNotification({
        type: 'error',
        title: 'ws-onerror',
        message: 'ws-onerror',
