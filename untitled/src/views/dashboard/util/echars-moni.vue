...
const generateRealtimeData = () => {
  ...
  for (let i = 0; i < 600; i++) {
    ...
    initialData.push({time, value: Number(value.toFixed(2))})
  }
  return initialData
}
...
const updateChart = () => {
  ...
  if (myChart.value && !isUserZooming.value) {
    if (!chartOption) {
      ...
      chartOption.series[0].data = realtimeData.value.map(d => [d.time.getTime(), d.value])
...
onMounted(() => {
  ...
  if (dom) {
    ...
    dataInterval = setInterval(() => {
      const lastValue = realtimeData.value[realtimeData.value.length - 1].value
      let newValue = lastValue + (Math.random() - 0.5) * 2
      if (isPercentType.value) newValue = Math.max(0, Math.min(100, newValue))
      ...
      if (realtimeData.value.length > 600) {
